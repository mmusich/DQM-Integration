...
* the `hlt_dqm_clientPB-live_cfg.py` reads the `streamDQMHistograms` proto-buffer file generated from run 402360 (from 2026B pp run, [OMS link](https://cmsoms.cern.ch/cms/runs/report?cms_run=398183&cms_run_sequence=GLOBAL-RUN)):
   ```
   run402360/run402360_ls0193_streamDQMHistograms_pid2453030.jsn
   run402360/run402360_ls0193_streamDQMHistograms_pid2453030.pb
   ```
...
The streamer files for the `streamDQMHistograms` were prepared using the following script:
```bash
#!/bin/bash -ex
